...
- [x] Simplify home/directory browsing (2026-09-07): put featured trackers before recent activity, show printing-reference thumbnails, add direct Report a Find links for live trackers, and fix narrow-screen heading/action wrapping. Golden Chocobo is now active and reference artwork stays separate from discovery evidence. At the owner's request, `/trackers` now has one top disclosure for all displayed merchants rather than one inside every tracker card; preserve individual tracker/reference/detail notices and affiliate destinations. The homepage has no affiliate placements.

Home/directory verification: 591 tests, lint and build pass; `check-directory-ui.mjs` passes at 320/390/768/1440px with real reference-image decoding, navigation checks, exact affiliate URL comparison, one visible top disclosure before all marketplace links, and overflow checks. Desktop/mobile screenshots inspected. Offline checks pass for 819 configured, 1,740 serial-boundary, 66 catalog and 894 printing affiliate links. No account, tracking-ID, safety-gate or storage changes.
...
- [x] Add a structured supported-source field to private Needs Info replies. Link-only replies are accepted without notes/photo, normalized and recorded immutably without overwriting original facts. Retry comparisons include the source, all reply sources are screened on approval/merge, and the owner checks a stored reply selection before opening. Approved sources append retractable card-history events. Backup validation preserves canonical sources and rejects ambiguous reply IDs. New replies permit at most eight distinct report sources; raw URLs typed into notes are not evidence.

Follow-up verification (2026-09-07): 591 tests across 36 files, lint and build pass. New `check-followup-ui.mjs` covers link-only/notes-only replies, empty/unsupported sources, disabled in-flight fields, lost-response retry, private receipt rendering and failed/successful stored-source review at 320/390/1440px. Existing owner/workflow suites also pass and screenshots were inspected. Offline affiliate checks pass unchanged. The deployment smoke suite now adds anonymous denial of source checks and both receipt methods (68 checks). No real provider scan, production discovery or credential change is part of this release; deferred service setup remains required.
