      ...
      <div className="grid grid-cols-1 gap-3 md:grid-cols-3">
        {resolvedLinks.slice(0, 3).map((link) => {
          ...
          return (
            ...
            >
              <span className="block text-xs font-bold uppercase tracking-wide opacity-75">{copy.eyebrow}</span>
              <span className="mt-1 block font-bold">{link.label}</span>
              <span className="mt-1 block text-xs opacity-80">{copy.detail}</span>
            </AffiliateOutboundLink>
          );
        })}
      </div>
