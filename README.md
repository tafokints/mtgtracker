...
Owner login requires explicit credentials; there is no default development password. Production also requires owner identity and an enrolled TOTP authenticator. Sessions are server-revocable and credential rotation invalidates them. Read `docs/SECURITY_OPERATIONS.md` before deploying; missing MFA intentionally disables production login.
