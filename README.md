...
- Planned catalog rows and entry pages include prefilled tracker-request links so collector interest can become actionable backlog signal
...
ADMIN_OWNER_ID=...
ADMIN_PASSWORD_FRONTEND=...
ADMIN_SESSION_SECRET=...
ADMIN_TOTP_SECRET=...
BLOB_STORE_ID=...
# Optional static-token alternative for Blob; required by the recovery runner.
BLOB_READ_WRITE_TOKEN=...
NEXT_PUBLIC_GOOGLE_SITE_VERIFICATION=...
```

Owner login requires explicit credentials; there is no default development password. `ADMIN_PASSWORD_FRONTEND` is a server-only secret despite its name and replaces `ADMIN_PASSWORD` without a fallback. Never expose it through `NEXT_PUBLIC_` or Next.js `env` configuration. Set it for Production (and separately for any Preview/Development owner access), then deploy the updated code. Production also requires owner identity and an enrolled TOTP authenticator. Sessions are server-revocable and credential rotation invalidates them. Read `docs/SECURITY_OPERATIONS.md` before deploying; missing MFA intentionally disables production login.
...
5. Connect a **private** Vercel Blob store using `BLOB_STORE_ID` and SDK-managed Vercel OIDC; a static `BLOB_READ_WRITE_TOKEN` is also supported. Never use a public store for new submissions or share Production stores with isolated Preview QA. Use the owner-only storage test and activation steps in `docs/INTAKE_SETUP.md`, then complete Turnstile and scanning setup before opening intake.
...
- Public discovery share URLs use the same campaign format, with copied public shares tracked as `public_copy`.
...
- Production authentication requires `ADMIN_PASSWORD_FRONTEND`, `ADMIN_SESSION_SECRET`, `ADMIN_OWNER_ID` and an enrolled `ADMIN_TOTP_SECRET`; none may fall back to development credentials. Login allows ten attempts per IP per fifteen minutes and requires Redis for rate limiting.
...
- Stats marketplace CTAs: tracker stats pages include tracked marketplace links, affiliate disclosure, and trust/market context with placement `tracker-stats-cta`.
...
- `npm run smoke` verifies public trust pages, sitemap coverage, breadcrumb structured data, tracker marketplace CTAs, and stats-page trust/market context.
