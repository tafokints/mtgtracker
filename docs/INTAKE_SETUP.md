...
Production uses the owner's existing site key `0x4AAAAAAEp4P0nETpWGpEVE`. Do not create a replacement widget, rotate its secret or change pre-clearance. The [Cloudflare Spin existing-widget flow](https://developers.cloudflare.com/turnstile/spin/prompt.md) was fetched and reviewed. Automatic secret recovery was stopped because no approved Wrangler 4.109+ installation was available; the owner instead used Vercel's normal secret-management interface. No Cloudflare credential, secret getter, helper installation or widget metadata mutation was used.
