...
6. A token holder can view a minimal receipt and answer a needs-info request with bounded plain text and owned attachments. New files use fresh one-hour upload permission for the original report. They still undergo the same image checks. Replying preserves original report facts and review history and returns the report to pending.
