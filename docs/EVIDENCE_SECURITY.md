...
6. Public submit binds uploaded IDs to the signed report and selected slot, then creates an immutable pending report. Reusing the same permission returns the original report ID without creating another discovery. Different card/serial selections require a new permission and new uploads.
...
8. Images remain in private storage after approval. The app serves them only when the asset is clean and both approved report lineage and canonical found-card references match. Responses are no-store and bytes must match the stored hash. Unpublishing/removing references prevents subsequent app reads, not retrieval of copies someone already downloaded.
